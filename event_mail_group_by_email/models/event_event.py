    ...
    def _mail_attendee_group(self, template, attendees, force_send=False):
        ...
        if other_attendees:
            reg_url = '<a href="#model=event.registration&id=%d">%s</a>' % (
                main_attendee.id,
                main_attendee.display_name,
            )
            body = _(
                "Communication <b>%(template_name)s</b> sent to attendee "
                "with the same email: %(url)s."
            ) % dict(template_name=template.display_name, url=reg_url)
