...
class PassLine(models.Model):
    ...
    attachment_ids = fields.Many2many('ir.attachment', 'pass_line_attachment_rel', 'pass_line_id', 'attachment_id', 'Attachments')
    ...
    def write(self, vals):
        if vals.get('validity_date'):
            ...
            if vals.get('validity_date') <= fields.Date.today():
                vals['state'] = 'valid'
            else:
                vals['state'] = 'draft'
    ...
    def action_send_multiple_by_mail(self, type, auto=False):
        records = self
        print(records)
        print(records[0])
        ...
        if type == 'pass':
            ...
            print("invitation_mail_template a envoyer \n \n", template)    
        ...
        if not auto:
            ...
            return self.env.ref('event_pass.invitation_mail_template').with_context(context).send_mail(records[0].id, force_send=True)
    ...
    def _get_pdf_data(self, records, type):
        if type == "pass":
            report = self.env.ref('event_pass.pass_report')
        else:
            report = self.env.ref('event_pass.invitation_report')
        pdf_data, _ = report._render_qweb_pdf(records.ids)
        return pdf_data

    def action_confirm(self):
        if self.filtered(lambda pass_line: pass_line.state != 'draft'):
    ...
    def _cron_check_validity(self):
        pass_lines = self.env['event.pass.line'].search(
            [('validity_date', '<=', fields.Date.today()), ('state', '=', 'draft')])
        pass_lines.write({'state': 'valid'})

        pass_lines = self.env['event.pass.line'].search(
            [('expiration_date', '<=', fields.Date.today()), ('state', '=', 'valid')])
